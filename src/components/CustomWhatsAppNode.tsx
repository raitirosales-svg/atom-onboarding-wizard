...
function getNodeConfigs(): Record<NodeType, NodeStyleConfig> {
  return {
    ...
    eval_response: {
      ...
      labelKey: 'nodeEvalResponseLabel',
    ...
    typification: {
      icon: CheckCircle2,
      labelKey: 'nodeTypificationLabel',
    ...
    customer_stage: {
      icon: Kanban,
      labelKey: 'nodeCustomerStageLabel',
...
export const CustomWhatsAppNode = memo(({ id, data, selected }: NodeProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <div className={`flex items-center justify-between rounded-t-xl px-3.5 py-2.5 ${styleConfig.headerBg}`}>
        ...
        <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium ${styleConfig.badgeBg}`}>
          {typeLabel}
        </span>
      </div>
      ...
      <div className="p-3 text-xs text-slate-600 dark:text-slate-300">
        ...
        {nodeType === 'save_field' && nodeData.fieldName && (
          <div className="mt-2 flex items-center justify-between rounded border border-indigo-200 bg-indigo-50/50 px-2 py-1 dark:border-indigo-900/50 dark:bg-indigo-950/30">
            <span className="font-mono text-[11px] font-medium text-indigo-700 dark:text-indigo-300">
              {t('nodeFieldLabel')} ${nodeData.fieldName}
            </span>
          </div>
        )}
        ...
        {nodeType === 'eval_response' && (
          <div className="mt-3 space-y-1.5 border-t border-slate-100 pt-2.5 dark:border-slate-800">
            ...
            {options.map((opt, idx) => (
              <div key={idx} className="relative flex items-center justify-between rounded border border-emerald-200 bg-emerald-50/70 py-1 pl-2 pr-4 text-[11px] font-medium text-emerald-800 dark:border-emerald-900/60 dark:bg-emerald-950/40 dark:text-emerald-200">
                <span className="truncate">{opt}</span>
                <Handle
                  type="source" position={Position.Right} id={`button_${idx}`}
            ...
            <div className="relative flex items-center justify-between py-0.5 text-[10px] text-slate-400">
              <span>{t('nodeEvalTimeout')}</span>
              <Handle type="source" position={Position.Right} id="no_answer"
        ...
        {nodeType === 'smarton' && (
          <div className="mt-3 space-y-1.5 border-t border-slate-100 pt-2.5 dark:border-slate-800">
            <div className="text-[10px] font-semibold text-fuchsia-500 uppercase">
              {t('nodeSmartonIntents')}
            </div>
            {options.map((opt, idx) => (
              <div key={idx} className="relative flex items-center justify-between rounded border border-fuchsia-200 bg-fuchsia-50/70 py-1 pl-2 pr-4 text-[11px] font-medium text-fuchsia-800 dark:border-fuchsia-900/60 dark:bg-fuchsia-950/40 dark:text-fuchsia-200">
                <span className="truncate">{opt}</span>
                <Handle
                  type="source" position={Position.Right} id={`intent_${idx}`}
                  style={{ top: `${28 + idx * 28}px`, right: '-14px' }}
